...
#define DQE_VER_500 (0x5000000) // 9xxx evt0
#define DQE_VER_600 (0x6000000) // 9xxx evt1
#define DQE_VER_610 (0x6100000) // 8xxx
	...
	if (dqe->colormode.dma_buf) {
		if (dqe->colormode.vaddr) {
			struct dma_buf_map old_map =
				DMA_BUF_MAP_INIT_VADDR(dqe->colormode.vaddr);
				...
				struct drm_crtc_state *crtc_state)
{
	...
	if (sizeof(ctx_from->disp_dither) && ctx_from->disp_dither[0]) {
	...
	if (sizeof(ctx_from->degamma_lut) && ctx_from->degamma_lut[0][0]) {
	...
	if (sizeof(ctx_from->regamma_lut) && ctx_from->regamma_lut[0][0]) {
	...
	if (sizeof(ctx_from->cgc17_con) && ctx_from->cgc17_con[0]) {
	...
	if (sizeof(ctx_from->hsc48_lut) && ctx_from->hsc48_lut[0]) {
				...
				struct drm_crtc_state *crtc_state)
{
	...
	if (unlikely(dqe->cfg.width != width) ||
		...
		dqe_debug(dqe, "w x h (%d x %d), bpc (%d/%d) rcd %d\n",
				dqe->cfg.width, dqe->cfg.height,
				dqe->cfg.in_bpc, dqe->cfg.out_bpc, dqe->cfg.rcd_en);
				...
				enum dqe_aps_type type)
{
	...
	// use same TDR_MAX/MIN/DSTEP for 8bit and 10bit
	if (IS_DQE_VER(dqe, DQE_VER_600))
		bpc = DQE_BPC_TYPE_10;
	// round off instead of 2 bit shift
				...
				struct dqe_ctx_int *lut, bool clrForced)
{
	...
	if (!sizeof(lut->scl_lut) || !sizeof(lut->scl_input))
		return;
			...
			struct dqe_ctx_int *lut, bool clrForced)
{
	...
	/* DQE0_DE_FLATNESS */
	dqe->ctx.de[3] = (
		DE_FLAT_EDGE_TH(de_lut[10]>>shift) | DE_EDGE_BALANCE_TH(de_lut[11]>>shift)
	);

	/* DQE0_DE_CLIPPING */
	dqe->ctx.de[4] = (
		DE_PLUS_CLIP(de_lut[12]>>shift) | DE_MINUS_CLIP(de_lut[13]>>shift)
	);

	/* DQE0_DE_GAIN */
	dqe->ctx.de[5] = (
		DE_GAIN(de_lut[14]) | DE_LUMA_DEGAIN_TH(de_lut[15]>>shift) | DE_FLAT_DEGAIN_FLAG(de_lut[16]) |
